  ...
  >
    ...
    <a-form :model="current_user" :label-col="{span: 6}" :wrapper-col="{span: 18}">
      <a-form-item label="编号">
        <a-input v-model:value="current_user.identity" :disabled="true"/>
      </a-form-item>
      <a-form-item label="姓名">
        <a-input v-model:value="current_user.name" />
      </a-form-item>
      <a-form-item label="密码" v-show="current_state === 'addForeignStaff'">
      ...
      <a-form-item label="身份证号">
        <a-input v-model:value="current_user.idCard" :disabled="current_state !== 'addForeignStaff'"/>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleModalOK = () => {
      ...
      axios.post("/foreign-staff/save", current_user.value,{
        headers:{
          'type': store.getters.getUser().type}
      }).then((response) => {
        const data = response.data;
    ...
    const edit = (record: any) => {
      ...
      current_state.value = "editForeignStaff";
    }
    ...
    const add = () => {
      current_user.value = {};
      axios.get("/foreign-staff/newIdentity").then((response)=>{
        const data = response.data;
        if(data.success){
          current_user.value.identity = data.content;
        }else{
          message.error(data.message);
        }
      })
      ...
      modalVisible.value = true;
      current_state.value = "addForeignStaff";
