<template>
  ...
  >
    ...
    <a-form :model="current_user" :label-col="{span: 6}" :wrapper-col="{span: 18}">
      ...
      <a-form-item label="密码" v-show="!editState">
      ...
      <a-form-item label="身份证号">
        <a-input v-model:value="current_user.idCard" :disabled="current_user.isStudent"/>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleQuery = (params: any) => {
      ...
      axios.get("/school-staff/teacher/list", {
        params: {
          page: params.page,
          size: params.size,
    ...
    const handleModalOK = () => {
      ...
      axios.post("/school-staff/teacher/save", current_user.value, {
        headers: {
          'type': store.getters.getUser().type,
        }
      }).then((response) => {
