        ...
        <template v-slot:action="{text, record}">
          <a-space size="small">
            ...
            <a-popconfirm
                ...
                ok-text="是"
                cancel-text="否"
                @confirm="handleDelete(record.counselorIdentity)" >
  ...
  >
    ...
    <a-form :model="current_user" :label-col="{span: 6}" :wrapper-col="{span: 18}">
      <a-form-item label="编号">
        <a-input v-model:value="current_user.counselorIdentity" :disabled="current_counselor_state !== 'addCounselor'"/>
      </a-form-item>
      <a-form-item label="姓名">
        <a-input v-model:value="current_user.name" />
      </a-form-item>
      <a-form-item label="密码" v-show="current_counselor_state === 'addCounselor'">
      ...
      <a-form-item label="身份证号">
        <a-input v-model:value="current_user.idCard" :disabled="current_counselor_state !== 'addCounselor'"/>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleModalOK = () => {
      ...
      axios.post("/counselor/save", current_user.value,{
        headers:{
          'type': store.getters.getUser().type}
      }).then((response) => {
        const data = response.data;
    ...
    const edit = (record: any) => {
      ...
      current_counselor_state.value = "editCounselor";
    }

    const current_counselor_state = ref();
    current_counselor_state.value = "editCounselor";
    // 新增
    const add = () => {
      modalVisible.value = true;
      current_user.value = {};
      current_counselor_state.value = "addCounselor";
    }
    ...
    const handleResetModalOK = () => {
      ...
      current_user.value.identity = current_user.value.counselorIdentity;
      console.log("current user: ", current_user.value);
